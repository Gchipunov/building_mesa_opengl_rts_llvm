...
struct MultiCommandPacket1 {   // Send multiple GAME_STATE Commands in one packet, maybe just position and velocity to pack more
 //   uint32_t num_commands;
    uint8_t num_commands;   // 256 possibilies , and probably won't, and shouldn't be over 255
...
struct TrainCommand {
    ...
    // uint32_t objectnumber_id;	// upgrade level
    uint32_t sequence_number;				// sequence number
    uint32_t train_object_id;   // unit ID
};

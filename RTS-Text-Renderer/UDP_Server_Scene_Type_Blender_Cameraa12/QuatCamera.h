...
#ifndef COLO_GAME_QUAT_CAMERA_MATRIX_H
#define COLO_GAME_QUAT_CAMERA_MATRIX_H


#include <GL/glew.h>
#include <GLFW/glfw3.h>

// #include "../math/gosha_math2.h"
// #include "../math/gosha_math3.h"
// 
...
//
// https://raw.githubusercontent.com/Gchipunov/gchipunov.github.io/refs/heads/main/QuatCamera/Mod6_2.html
#include <map>
#include <string>
#include <algorithm> // For std::min and std::max
    ...
    /**
 * @struct MouseState
 * @brief Holds the current state of mouse input.
 */
struct MouseState {
    bool isDown;
    float lastX;
...
/**
 * @struct TouchState
 * @brief Holds the current state of touch input.
 * The identifier can be used to track a specific touch point.
 */
struct TouchState {
    long identifier; // Using long to match JS touch.identifier
    float lastX;
    float lastY;
};
...
/**
 * @struct InputState
 * @brief A container for all input device states.
 */
struct InputState {
    MouseState mouse;
    TouchState touch;
    // Maps a key name (e.g., "forward") to its pressed state (true/false).
    std::map<std::string, bool> keys;

    // Constructor to set initial values
    InputState()
        : mouse({ false, 0.0f, 0.0f }),
        touch({ -1L, 0.0f, 0.0f }) { // -1 to represent no active touch, like null in JS
    }
};


class ColoQuatCamera {
public:
    ColoQuatCamera(int width, int height, GLFWwindow* Mainwindow);
    ~ColoQuatCamera();
    ...
    // The input state is now a member of the camera class.
    InputState inputState;

    // World constants
    Vec3 WORLD_UP;//= vec3.create(0, 0, 1); // Z is up!
    Vec3 WORLD_FORWARD;//= vec3.create(0, 1, 0);
    Vec3 WORLD_RIGHT;//= vec3.create(1, 0, 0);
    ...
    // The camera needs a reference to the window for input handling.
    GLFWwindow* window;
    ...
    Mat4 GetProjectionMatrix() { return perpective; };

    glm::mat4 GetGLMViewMatrix();
    //{        };
    glm::mat4 GetGLMProjectionMatrix();
    //{    };
    ...
    void keyCallbackDebug(GLFWwindow* window, int key, int scancode, int action, int mods);

    void lookAt2DRect(float x, float y, float width, float height);
    void lookAt2DRect2(float x, float y, float width, float height);

    void updateDebug2(float deltaTime);

};
extern ColoQuatCamera* g_ColoQuatCamera;

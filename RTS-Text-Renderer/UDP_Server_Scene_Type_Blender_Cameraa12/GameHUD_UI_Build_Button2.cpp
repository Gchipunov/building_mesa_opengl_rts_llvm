...
void push_vertex_gameui_button2(std::vector<float>& vertices, float x, float y, float z, float u, float v, float vvtype) {
...
GameHUD_UI_Build_Button2::GameHUD_UI_Build_Button2()
{
...
int GameHUD_UI_Build_Button2::CreateMainMenuQuads()
{
	...
	for (int i = 0; i < 1; i++) // for (int i = 0; i < 4; i++) make 4 quads on top of each other
	{
		...
		float fStartY = 0.45 + margin - (0.25f * 1.0f);;//0.10f + (fIncreamentY * (float)i);
	...
	//	float fEndY = 0.20f + fStartY; // +(fIncreamentY * (float)i); // 20% Percent from left side of the screen
		...
		push_vertex_gameui_button2(mainVertices, fEndX, fStartY, z, 1.0f, 1.0f, vvtype); // bottom-left screen position, new bottom-left UVs (from old top-left)
		...
		push_vertex_gameui_button2(mainVertices, fEndX, fEndY, z, 1.0f, 0.0f, vvtype); // bottom-right screen position, new bottom-right UVs (from old bottom-left)
		////	mainVertices.push_back(1);

			// Vertex 3: Original top-right screen position. Should get the new top-right UVs.
		push_vertex_gameui_button2(mainVertices, fStartX, fEndY, z, 0.0f, 0.0f, vvtype); // top-right screen position, new top-right UVs (from old bottom-right)
		...
		push_vertex_gameui_button2(mainVertices, fStartX, fStartY, z, 0.0f, 1.0f, vvtype); // top-left screen position, new top-left UVs (from old top-right)
...
void GameHUD_UI_Build_Button2::UpdateMainMenu()
{
	/*
...
void GameHUD_UI_Build_Button2::mouseCallback(GLFWwindow* window, double xpos, double ypos)
{

}
void GameHUD_UI_Build_Button2::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
...
GameHUD_UI_Build_Button2* g_ANA_GameHUD_UI_Build_Button2;

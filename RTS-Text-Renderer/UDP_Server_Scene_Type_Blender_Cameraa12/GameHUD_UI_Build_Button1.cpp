...
// https://www.beyondallreason.info/map/dsd-8-way
// Add Map loader.
...
void push_vertex_gameui_button1(std::vector<float>& vertices, float x, float y, float z, float u, float v, float vvtype) {
...
GameHUD_UI_Build_Button1::GameHUD_UI_Build_Button1()
{
...
int GameHUD_UI_Build_Button1::CreateMainMenuQuads()
{
	float vvtype = 1020.0f;

	int iStartMainMenuUI_Index = mainVertices.size();
	iMainMenuUI_MainVertices_index = mainVertices.size();
	for (int i = 0; i < 4; i++) // for (int i = 0; i < 4; i++) make 4 quads on top of each other
	{
		//float fIncreamentY = 0.20f;
		float margin = ((i + 1) * 0.01f);
		float margin2 = ((i + 2) * 0.01f);

		float fStartX = 0.70f + margin; // 20% Percent from left side of the screen
		//	float fStartY = 0.20f +  (fIncreamentY * (float)i); // 20% Percent from left side of the screen
		float fStartY = 0.45 + margin;//0.10f + (fIncreamentY * (float)i);

		float fEndX = 0.900f - margin; // 20% Percent from left side of the screen
		float fEndY = 0.20f - margin2 + fStartY; // +(fIncreamentY * (float)i); // 20% Percent from left side of the screen
		// - (margin )
	...
	//	float fEndY = 0.20f + fStartY; // +(fIncreamentY * (float)i); // 20% Percent from left side of the screen
		...
		float z = -0.01f + (i * -0.01f);
		if (i == 3)
		{
			vvtype = 1021.0f;
			printf("INSERT TEXT RENDERER AND SPRIATE RENDERER HERE!! HAPPY NEW YEARS\n");  // 1/4/2026

		//	uvx0 = g_pTextureSpriteAltasLoader->spriteMap["MCV1_Structure_64x64.png"].x;
		//	uvx1 = g_pTextureSpriteAltasLoader->spriteMap["MCV1_Structure_64x64.png"].x + g_pTextureSpriteAltasLoader->spriteMap["MCV1_Structure_64x64.png"].width;
		//	uvy0 = g_pTextureSpriteAltasLoader->spriteMap["MCV1_Structure_64x64.png"].y;
		//	uvy1 = g_pTextureSpriteAltasLoader->spriteMap["MCV1_Structure_64x64.png"].y + g_pTextureSpriteAltasLoader->spriteMap["MCV1_Structure_64x64.png"].height;
			 uvx0 = 0.0f;
			 uvx1 = 1.0f;
			 uvy0 = 0.0f;
			 uvy1 = 1.0f;
		...
		push_vertex_gameui_button1(mainVertices, fEndX, fStartY, z, uvx1, uvy1, vvtype); // bottom-left screen position, new bottom-left UVs (from old top-left)
		...
		push_vertex_gameui_button1(mainVertices, fEndX, fEndY, z, uvx1, uvy0, vvtype); // bottom-right screen position, new bottom-right UVs (from old bottom-left)
		////	mainVertices.push_back(1);

			// Vertex 3: Original top-right screen position. Should get the new top-right UVs.
		push_vertex_gameui_button1(mainVertices, fStartX, fEndY, z, uvx0, uvy0, vvtype); // top-right screen position, new top-right UVs (from old bottom-right)
		...
		push_vertex_gameui_button1(mainVertices, fStartX, fStartY, z, uvx0, uvy1, vvtype); // top-left screen position, new top-left UVs (from old top-right)
...
void GameHUD_UI_Build_Button1::UpdateMainMenu()
{
	/*
...
void GameHUD_UI_Build_Button1::mouseCallback(GLFWwindow* window, double xpos, double ypos)
{

}
void GameHUD_UI_Build_Button1::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
...
GameHUD_UI_Build_Button1* g_ANA_GameHUD_UI_Build_Button1;
